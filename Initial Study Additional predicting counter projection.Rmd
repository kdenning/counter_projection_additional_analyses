...
clean_data2 <- clean_data %>%
  ...
  mutate(inoutgroup_effects = ifelse(inoutgroup == "Ingroup", .5,
         ...
         MFQStereo = ifelse(targ_cand == "Clinton", MFQClin, 
                            ifelse(targ_cand == "Trump", MFQTrump, NA))) %>% 
  #grand mean centering MFQ for self
  mutate(MFQSelf_c = MFQSelf - mean(MFQSelf)) %>%  
  #removing people who failed the manipulation check
  filter(targ_cand == "Clinton" & Man_TargVote == 1 | 
           targ_cand == "Trump" & Man_TargVote == 2) %>%
  select(-Day_content, -Day_key) %>% 
  #effects coding candidate target voted for
  mutate(Targ_Cand_effectscoded = ifelse(targ_cand == "Clinton", .5, 
                                         ifelse(targ_cand == "Trump", -.5, NA)),
...
## Stereotyping by projection plots
...
### By in/out-group and identification
...
### By Candidate and identification
...
As stereotype difference increases, projection decreases more for Clinton supporters than Trump supporters.
