...
nvi_projxdiffxident$ident_c <- as.factor(nvi_projxdiffxident$ident_c)
...
ggplot(nvi_projxdiffxident, aes(MFQSelf_c, 
  ...
  geom_smooth(method = "lm", se = FALSE, size = .7, colour = "black",
              aes(linetype = stereo_diff)) +
  facet_wrap(~ident_c, labeller = labeller(ident_C = ident_label)) +
  scale_linetype_manual("Stereoype \nDifference", breaks = c(0,1.0,2.0,3.0,4.0), 
                       labels = c("None",
                                  "1.0",
                                  "2.0",
...
Not sure why labeller decided to stop working... need to figure this out!!!
