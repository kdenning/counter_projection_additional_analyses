...
ggplot(proj_stereo_only, aes(MFQSelf_c, 
                             fit, 
                             group = Stereotyping, 
                             color = Stereotyping)) +
  ...
  scale_linetype_manual("Stereotyping", breaks = c(-1.35, 0, 1.35), 
                       ...
                       values = c("solid", 
                                  ...
                                  "dotted")) +
  theme_minimal() +
  labs(title = "Projection predicted by stereotyping",
...
# Stereotype constant: Follow-up 1

```{r stereo constant f1}

f1_constant_data <- pptnpf1_comb %>% 
  group_by(Condition, Issue_agreement) %>% 
  mutate(outgroup_constant = mean(MFQTarg))

f1_constant <- lmer(MFQTarg ~ MFQSelf_c * outgroup_constant *
                                    Condition * Identification +
                                    (MFQSelf_c|Subject), 
                                 data = f1_constant_data)

tab_model(f1_constant,
          title = "Projection by condition including constant")
```
