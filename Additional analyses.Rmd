...
```{r stereo analysis}
#model
projmodel_stereo_included <- lmer(MFQTarg ~ MFQSelf_c * MFQStereo_c *
...
```{r}
describe(clean_data2$ident_c)
describe(clean_data2$inoutgroup_effects)
...
mfq_binding <- clean_data2 %>% 
  ...
  select(subid, MFQ_Number, MFQTarg, MFQSelf_c, MFQStereo_c, inoutgroup_effects, ident_c) %>% 
  unique() %>% 
  na.omit()
